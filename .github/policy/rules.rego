...
deny[msg] {
  ...
  not service_labels
  msg = sprintf("Service %s should set app or app.kubernetes.io/name label selector", [name])
}
...
deny[msg] {
  ...
  not match_labels
  msg = sprintf("Statefulset %s should set app or app.kubernetes.io/name label selector", [name])
}
